...
class Rect:
    ...
    def get_trans_points(self, mat3x3):
        """ Return transfomed rectangle points in a clockwise order
        staring from the top left point
        """
        pts = []
        pt = np.array([[self.left], [self.top], [1]])
        npt = np.dot(mat3x3, pt)
        pts.append((npt[0, 0] / npt[2, 0], npt[1, 0] / npt[2, 0]))
        pt[0, 0] = self.right
        npt = np.dot(mat3x3, pt)
        pts.append((npt[0, 0] / npt[2, 0], npt[1, 0] / npt[2, 0]))
        pt[1, 0] = self.bottom
    ...
    def get_trans_poly(self, mat3x3):
        pts = self.get_trans_points(mat3x3)
        return Polygon(pts)

    def get_trans_regpoly(self, mat3x3, cut):
        pts = self.get_trans_points(mat3x3)
        nPts = ((np.round(pts[0][0]), np.round(pts[0][1])),
                (np.round(pts[1][0]) - cut, np.round(pts[0][1])),
                (np.round(pts[2][0]) - cut, np.round(pts[2][1])),
                (np.round(pts[0][0]), np.round(pts[2][1])))

        return Polygon(nPts)

    def get_poly(self):
        pts = ((self.left, self.top),
                (self.right, self.top),
